...
public:
  virtual double value(const Point<dim> &p,
    ...
    double exponent = -(x_diff * x_diff + y_diff * y_diff) / (2.0 *
    sigma_squared); return 0.3*std::exp(exponent);*/
    float time_component =
        ((this->get_time() * 1e-8 - 2.5)) / 97.5; // from 0 to 1
    float modulo_time_component = fmod((time_component / 3), 6);
    float center_y; // = 3.5e6 *(1-time_component) + 3.6e6 *(time_component) ;
                    // //= 3600000.
    float center_x; // = 5.4e6 *(1-time_component) + 5.5e6 *(time_component);
                    // //5400000.
    ...
    else if ((modulo_time_component > 1) &&
             (modulo_time_component <= 2)) // TODO
    {
      center_y = 3.45e6 * (2 - modulo_time_component) +
                 3.6e6 * (modulo_time_component - 1); //= 3600000.
      center_x = 5.4e6 * (2 - modulo_time_component) +
                 5.5e6 * (modulo_time_component - 1); // 5400000.
    } else if ((modulo_time_component > 2) &&
               (modulo_time_component <= 3)) // TODO
    {
      center_y = 3.6e6; //= 3600000.
      center_x = 5.6e6; // 5400000.
    } else if ((modulo_time_component > 3) && (modulo_time_component <= 4)) {
      center_y = 3.6e6 * (4 - modulo_time_component) +
                 3.45e6 * (modulo_time_component - 3); //= 3600000.
      center_x = 5.6e6 * (4 - modulo_time_component) +
                 5.8e6 * (modulo_time_component - 3); // 5400000.
    } else if ((modulo_time_component > 4) &&
               (modulo_time_component <= 5)) // TODO
    {
      center_y = 3.45e6; //= 3600000.
      center_x = 5.8e6;  // 5400000.
    } else if ((modulo_time_component > 5) && (modulo_time_component <= 6)) {
      center_y = 3.45e6 * (6 - modulo_time_component) +
                 3.45e6 * (modulo_time_component - 5); //= 3600000.
      center_x = 5.8e6 * (6 - modulo_time_component) +
                 5.4e6 * (modulo_time_component - 5); // 5400000.
    }

    if ((p[0] > center_y && p[0] < center_y * 1.01) &&
        (p[1] > center_x && p[1] < center_x * 1.01))
    // if ((p[0]> 3600000. && p[0] < 3700000.) && (p[1] > 5400000. && p[1] <
    // 5500000.))
    {
      if (fmod(time_component, 3) <= 1) {
        return -(std::sin(modulo_time_component * 5 * numbers::PI) * 16 +
                 16.0) *
               1e-10;
      } else if ((fmod(time_component, 3) > 1) &&
                 (fmod(time_component, 3) <= 2)) {
        return -(std::sin(modulo_time_component * 2 * numbers::PI) * 10 + 12) *
               1e-10;
      } else if ((modulo_time_component > 2) && (modulo_time_component <= 3)) {
        return -(std::sin(modulo_time_component * 3 * numbers::PI) * 13 + 13) *
               1e-10; // this varies between 6.5 and 10.5
      }
      return -7e-10;
    } else
      return 0;
...
Tensor<1, dim> AdvectionField<dim>::value(const Point<dim> &p) const {
  ...
  if ((time_component / 18) <= 1) {
    value[0] = 0.00005 * (1); //*time_factor;//2; this is y-axis //
                              //((std::cos(2* time_component * numbers::PI)) -1)
    value[1] = -0.00005;
  } else if (((time_component / 18) > 1) && ((time_component / 18) <= 2)) {
    value[0] = -0.00002 *
               ((p[1] * 1e-6 * 5) - 27.5); //*time_factor;//2; this is y-axis
    value[1] = 0.00003 * (2);
  } else if (((time_component / 18) > 2) && ((time_component / 18) <= 3)) {
    value[0] =
        0.00005 * ((p[1] * 1e-6 * 5) - 27.5); //*time_factor;//2; this is y-axis
    value[1] = 0.00005;
  }
  return value;
}
...
WaveEquation<dim>::WaveEquation()
    : fe(1), dof_handler(triangulation), time_step(1000000000. / 8) // 1e9
      ,
...
template <int dim> void WaveEquation<dim>::output_results() const {
  ...
  const std::string filename_h5 = "solution-" + std::to_string(1) + "_" +
...
template <int dim> void WaveEquation<dim>::run() {
  ...
  for (; time <= 54 * 1e10; time += time_step, ++timestep_number) // 54 = 3*6*3
  {
    ...
    // diffusion_alpha.set_time(time); //implicit and explicit at different
    // times?
        ...
        for (const unsigned int i : fe_values.dof_indices()) {
          ...
          cell_rhs(i) +=
              ((fe_values.shape_value(i, q_index) *
               ...
               (time_step * diffusion_alpha.value(x_q) // TODO  check?
                * theta * fe_values.shape_grad(i, q_index) *
                old_solution_gradients[q_index]) -
               (time_step * theta * fe_values.shape_value(i, q_index) *
