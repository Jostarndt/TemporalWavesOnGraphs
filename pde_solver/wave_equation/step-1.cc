...
namespace Step23
{
  ...
  public:
                         ...
                         const unsigned int component = 0) const override
    {
      ...
      else if((this->get_time() >= 1000000.) && (this->get_time() <= 1100000.) && (p[1] > 6200000. ) && (p[0] > 3800000. ))
        return std::sin(this->get_time() * 0.00004 * numbers::PI);
      else
        return 0;
  ...
  public:
                         ...
                         const unsigned int component = 0) const override
    {
      ...
      if ((this->get_time() <= 50000.) && (p[1] > 6200000. ) && (p[0] < 3400000. ))
        return (std::cos(this->get_time() * 0.00004 * numbers::PI) * 0.00004 * numbers::PI);
      else if((this->get_time() >= 1000000.) && (this->get_time() <= 1100000.) && (p[1] > 6200000. ) && (p[0] > 3800000. ))
        return (std::cos(this->get_time() * 0.00004 * numbers::PI) * 0.00004 * numbers::PI);
      else
        return 0;

      // v(x, t) = 50000 * cos(4 * t * π) * 4 * π * 0.00004 for all points on the boundary
      // return (std::cos(this->get_time() * 0.00004 * numbers::PI) * 0.00004 * numbers::PI);
  ...
  void WaveEquation<dim>::setup_system()
  {
    ...
    for (auto &face : triangulation.active_face_iterators())
    {
	    if (face->at_boundary())
	    {
				    if ((face->center()[1] <= 6.2e6 ) &&
						    (face->center()[0] >= 3.37e6) &&
  ...
  void WaveEquation<dim>::output_results() const
  {
    ...
    const std::string filename_h5 = "solution-"  + Utilities::int_to_string(timestep_number, 3) +   ".csv";
    ...
    {
	    std::cerr << "Unable to open file Germany_coastline_points.csv" << std::endl;
    }
    ...
    for (int i = 0; i < 325; i ++)
    {
	    ...
	    //VectorTools::point_value(dof_handler, solution_u, Point<2>(nuts_coords[i][0], nuts_coords[i][1]), point_value);
	    try{
	    VectorTools::point_value(dof_handler, solution_u, Point<2>(nuts_coords[i][0], nuts_coords[i][1]), point_value);
  ...
  void WaveEquation<dim>::run()
  {
    ...
    for (; time <= 1500000; time += time_step, ++timestep_number) //500000 -> 320. For 
      {
