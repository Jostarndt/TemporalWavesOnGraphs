...
double diffusion_coefficient(const Point<dim> &p, double &time,
                             int setting_param) {
  // return 0; can be used to test it as an ODE-solver / is a const.
...
double r_value(const Point<dim> &p, double &time, int exp_param) {
  ...
  case 2:
    return 0.7 +
           (0.6 * std::sin(0.000005 * numbers::PI * p[0]) *
            std::sin(0.000005 * numbers::PI * p[1])); // space dependent r_value
  case 3:
    return 0.5 +
           (0.3 * std::sin(0.000002 * numbers::PI * p[0]) *
            std::sin(0.000005 * numbers::PI * p[1])); // space dependent r_value
  case 4:
    return 1.1 +
           (0.1 * std::sin(0.000008 * numbers::PI * p[0]) *
            std::sin(0.000005 * numbers::PI * p[1])); // space dependent r_value
  default:
...
/*The InitialValuesSusceptible class will decide
...
public:
  ...
  static double scen_one(const Point<dim> &p,
    ...
    {
      return 0.1 +
             (0.1 * std::sin(0.000005 * numbers::PI * p[0]) *
              std::sin(0.000005 * numbers::PI * p[1])); // TODO: 1- for infected
    } else if (component == 1)                          // Infected
    {
      return 0.1 -
             (0.1 * std::sin(0.000005 * numbers::PI * p[0]) *
  ...
  static double scen_two(const Point<dim> &p,
    ...
    {
      return 0.15 +
             (0.15 * std::sin(0.000005 * numbers::PI * p[0]) *
              std::sin(0.000005 * numbers::PI * p[1])); // TODO: 1- for infected
    } else if (component == 1)                          // Infected
    {
      return 0.15 -
  ...
  static double scen_three(const Point<dim> &p,
    ...
    {
      return 0.1 +
             (0.1 * std::sin(0.000004 * numbers::PI * p[0]) *
              std::sin(0.000005 * numbers::PI * p[1])); // TODO: 1- for infected
    } else if (component == 1)                          // Infected
    {
      return 0.1 -
  ...
  static double scen_four(const Point<dim> &p,
    ...
    if (component == 0) // Susceptible
    {
      return 0.1 +
             (0.1 * std::sin(0.000005 * numbers::PI * p[0]) *
              std::sin(0.000002 * numbers::PI * p[1])); // TODO: 1- for infected
    } else if (component == 1)                          // Infected
    {
      return 0.1 -
             (0.1 * std::sin(0.000005 * numbers::PI * p[0]) *
  ...
  // Here only, the value function gets called. It then calls function
  // option_function is pointing to
...
// ++++Boundary condition ++++
// works similar to the InitialValuesSusceptible class
...
std::cout << "   Equation: " << solver_control.last_step()
      << " CG iterations. RHSs L2-norm: " << system_rhs.l2_norm()<<"Newton
updates l2 norm: "<< newton_update.l2_norm() <<std::endl;
      */
...
template <int dim> void WaveEquation<dim>::run() {
  ...
  for (; time <= 91; time += time_step, ++timestep_number) {
    ...
    std::vector<Tensor<1, dim>> euler_old_susceptible_gradients(
        euler_fe_values
            .n_quadrature_points); // now std:.vector<std::vector<Tensor<1,
                                   // dim>>>
    std::vector<double> euler_old_susceptible_values(
        euler_fe_values.n_quadrature_points); // now std::vector<Vector<double>>
                                              //
        ...
        for (const unsigned int i : euler_fe_values.dof_indices())
          euler_cell_rhs(i) +=
              ((euler_fe_values[susceptible_extr].value(i, q) *
                ...
                * time_step * euler_old_susceptible_values[q] *
               ...
               (time_step * alpha_duration * (1 - theta) *
                euler_fe_values[infected_extr].value(i, q) *
                euler_old_infected_values[q]) -
               (time_step * current_diff_coeff // diffusion_d
                * (1 - theta) * euler_fe_values[infected_extr].gradient(i, q) *
                euler_old_infected_gradients[q])) *
              euler_fe_values.JxW(
                  q); // JxW for FESystems works apparently like this?
      }
    ...
    do {
      ...
      using CopyData =
          MeshWorker::CopyData<1, 1, 1>; //<2,2,2> will <n_matrices, n_vectors,
                                         // n_dof_indices>
          ...
          for (const unsigned int i : fe_values.dof_indices()) {
            residual_ad[i] +=
                ((fe_values[susceptible_fe].value(i, q) *
                 ...
                 (time_step * current_diff_coeff_two // diffusion_d
                  * theta * fe_values[susceptible_fe].gradient(i, q) *
                  old_susceptible_gradients[q])
                 // second equation
                 +
                 (fe_values[infected_fe].value(i, q) * old_infected_values[q]) -
                 (theta * current_transmission_r_two // transmission_r
                  * time_step * fe_values[infected_fe].value(i, q) *
                  old_infected_values[q] * old_susceptible_values[q]) +
                 (time_step * alpha_duration * theta *
                  fe_values[infected_fe].value(i, q) * old_infected_values[q]) +
                 (time_step * current_diff_coeff_two // diffusion_d
                  * theta * fe_values[infected_fe].gradient(i, q) *
